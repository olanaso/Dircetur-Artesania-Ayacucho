...
function eventEnviarCompra() {
  $("#btnfinalizarcompra").on("click", async function (e) {
    ...
    try {
      let resultado = await registrarPedidoCompra(
        correos,
        datosCliente,
        pedido
      );
      if (resultado) {
        ...
        location.href =
          "finalizacion-de-compra.html?pedido=" + resultado.num_pedido;
      }
    } catch (error) {
...
function eliminarCarritoCompras(artesano_id) {
  ...
  const artesaniasFiltradas = artesanias.filter(
    (artesania) => artesania.id_artesano != artesano_id
  ); // Filtrar el producto a eliminarnue
...
function eventDesactivarSubida() {
  ...
  document
    .getElementById("directPaymentCheck")
...
function handleUploadResponseimgprincipal(response) {
  $("#viewComprobanteBtn").attr(
    "href",
    getBasePathWithPort() + "/" + response.path
  );
  urlFileComprobante = getBasePathWithPort() + "/" + response.path;
  $("#viewComprobanteBtn").show();
}

// Agregar evento para mostrar el modal en clic sobre "Ver comprobante"
document
  .getElementById("viewComprobanteBtn")
  .addEventListener("click", function () {
    // Actualizar src de la imagen del modal con la URL del comprobante
    document.getElementById("comprobanteModalImg").src = urlFileComprobante;
  ...
  for (const oferta of product.objeto.oferta) {
    ...
    if (currentDate >= startDate && currentDate <= endDate) {
      if (
        !bestOffer ||
        endDate - startDate >
          new Date(bestOffer.fechaFin) - new Date(bestOffer.fechaInicio)
      ) {
...
function showCuentasArtesano(datos) {
  ...
  datos.forEach((dato) => {
    ...
    small.innerHTML = `Titular: <strong>${
      dato.Titular
    }</strong><br>Nº cuenta: <strong>${
      dato.Corriente
    }</strong><br>CCI: <strong>${dato.Interbancaria || "N/A"}</strong>`;
...
<tr>
									<td class="d-flex align-items-center">
										<img style="height: 100px;    max-height: 100%;   max-width: 100%;    object-fit: cover;" src="${
                      artesania.objeto.imagen_principal ||
                      "https://via.placeholder.com/80"
                    }" class="rounded mr-3" alt="Producto 1">
