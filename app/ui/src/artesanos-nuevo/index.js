...
async function buscarUsuario() {
  $("#btnvalidar").on("click", async function (e) {
    ...
    if (contrasena.trim() === "") {
      ...
      showToast(
        "El contraseña solo puede contener letras, números y guiones bajos"
      );
      //usuarioError.textContent = "El usuario solo puede contener letras, números y guiones bajos";
      errores = true;
      validar = 0;
  ...
  $("#btnguardarcambio").on("click", async function (e) {
    ...
    if (!isValid) {
      ...
      if (!ruc) {
        showToast("El RUC es obligatorio");
        return;
      }
...
async function editarArtesano(id) {
  ...
  lstreconocimientos.forEach((item) => {
    ...
    let nuevaFila = `   
              ...
              `;
    document
      .getElementById("listaReconocimiento")
      .insertAdjacentHTML("beforeend", nuevaFila);
  });
...
function disabilitar() {
  $("#home-tab2, #home-tab3, #home-tab4, #home-tab5, #home-tab6").addClass(
    "disabled"
  );
}
...
$(document).ready(function () {
  ...
  document.getElementById("guardarreconocimientoBtn").style.display =
    "inline-block";
  ...
  $("#listaMediopago").on("click", ".btn-edit-Mediopago", function () {
    ...
    document.getElementById("editarMediopagoBtn").style.display =
      "inline-block";
  ...
  $("#editarMediopagoBtn").on("click", function () {
    ...
    document.getElementById("guardarMediopagoBtn").style.display =
      "inline-block";
    document.getElementById("editarMediopagoBtn").style.display = "none";
  ...
  let filaActual;
  var addreconocimmientoModal = document.getElementById(
    "addreconocimmientoModalLabel"
  );
  var estadoreconocimmiento = 0;
  // Delegar el evento de clic para el botón editar
  $("#listaReconocimiento").on(
    "click",
    ".btn-edit-reconocimiento",
    function () {
      filaActual = $(this).closest("tr");
      ...
      document.getElementById("guardarreconocimientoBtn").style.display =
        "none";
      document.getElementById("editarreconocimientoBtn").style.display =
        "inline-block";
...
function handleUploadResponselistavideo(response) {
  $("#videoPreview")
    .attr("src", getBaseUrl(baseUrl) + "/" + response.path)
    .show();
}
...
$("#btnbuscarReniec").on("click", async function (e) {
  ...
  if (valor == 8) {
    ...
    if (artesanosDNI != null) {
      ...
      apellidosElement.value =
        artesanosDNI.apellidoPaterno + " " + artesanosDNI.apellidoMaterno;
    } else {
